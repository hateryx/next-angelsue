const constantsLotion = [
  {
    id: 20001,
    productName: "Magic Lotion",
    brand: "Angel Sue",
    image: "assets/products/lotion/angel2.jpeg",
    cost: "$98.00",
    summary:
      "Enriched with a blend of high-quality botanical extracts, vitamins, and antioxidants, our Angel Sue magic beauty lotion is designed to deeply moisturize and protect your skin. The luxurious texture glides on smoothly and absorbs quickly, providing long-lasting hydration without leaving a greasy or heavy residue.",
    details:
      "Our beauty lotion is perfect for anyone looking to elevate their skincare routine with a touch of luxury. The elegant packaging and delicate fragrance make it a perfect addition to any vanity or bathroom. Plus, it's suitable for all skin types, including sensitive skin. Experience the luxurious benefits of our beauty lotion and indulge in a daily ritual that will leave you feeling pampered and radiant. Treat yourself to the ultimate skincare experience and let your skin glow with health and vitality.",
    howToUse:
      "The first step in your routine, use morning and night, followed by your toner and moisturiser. Easy to use.",
  },
  {
    id: 20002,
    productName: "Cleanse & Polish Hot Cloth Cleanser",
    brand: "Liz Earle",
    image: "assets/products/lotion/2.jpeg",
    cost: "$98.00",
    summary:
      "On a day-to-day basis, if I’m out in the office or meeting up with friends, I love to wear a full-face beat. So, when coming home after a long day of said outings, I don’t want to be spending another hour lugging off all that makeup with a million cotton pads. That’s why this Cleanse & Polish captured my heart – it’s suuuper convenient. Just apply two pumps onto your dry makeup-full face and it’ll melt away everything in a tick.",
    details:
      "Enriched with sustainably sourced cocoa butter to soften, smooth and moisturise, and rosemary, chamomile and eucalyptus to tone, soothe and purify – this rich, indulgent cream deeply cleanses to instantly remove daily grime, pollutants and make up, even stubborn mascara, to leave skin feeling comfortable, balanced and exceptionally clean.",
    howToUse:
      "The first step in your routine, use morning and night, followed by your toner and moisturiser. Easy to use.",
  },
  {
    id: 20003,
    productName: "Calming Serum",
    brand: "Dr. Barbara Sturm",
    image: "assets/products/lotion/3.jpg",
    cost: "$98.00",
    summary:
      "Enriched with an active, plant-based complex of Cardiospermum, Echium Plantagineum and Sunflower, Dr. Barbara Sturm’s CALMING SERUM soothes and rebalances irritated skin.",
    details:
      "Perfect for soothing your complexion after sun exposure, sleep loss, jet lag, air conditioning or environmental pollution. When used over time this serum strengthens the skin’s natural defenses minimizing the skin’s sensitized reaction.",
    howToUse:
      "After cleansing, apply to the face. Wait for the serum to absorb before applying Dr. Barbara Sturm's FACE CREAM. To reduce skin sensitivity, use regularly in the morning and/or evening.",
  },
  {
    id: 20004,
    productName: "Oat Cleansing Balm",
    brand: "The INKEY List",
    image: "assets/products/lotion/4.jpg",
    cost: "$98.00",
    summary:
      "A rich cleansing balm that melts away makeup and impurities without drying the skin.",
    details:
      "This luxurious The INKEY List Oat Cleansing Balm gently dissolves make-up, cleans, and nourishes the skin, making it ideal for sensitive skin types.  Combining soothing colloidal oatmeal with ceramides to support skin’s natural protective barrier, it's also formulated with oat kernel oil.",
    howToUse:
      "The ideal gentle AM and PM cleanser. Gently massage a small amount onto dampened face and neck, add water, then rinse thoroughly.For an extra nourishment boost, leave on for 10 minutes before rinsing.",
  },
];

export default constantsLotion;
